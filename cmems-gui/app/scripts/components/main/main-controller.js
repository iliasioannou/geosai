			...
			if (url) {
				...
				$http.get(url)
					.then(function(response) {
						...
						if (response.data.features && response.data.features.length===0){ //HTTP STATUS == 200 -- no features returned or "ServiceException"
							...
							var obj = {
								...
								"features" : (response.data.features.length>0) ? response.data.features : null,
								"unit": olLayer.unit
							};
							if (resultObj!==""){
			...
			if ((info!==null) && info.deals && (info.deals.length>0)){
				angular.forEach(info.deals,
					function(item) {
						...
						if(item.service_type.indexOf("saimon")>-1){
							userDeals.push({
